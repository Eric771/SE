...
╚═════════════════════════
"""
vip="u186fe987ceb445ff5f001990fd583ff2"
...
Bots=[mid]
Creator=["u30a77719df29231e45210d278bcfdb0b"]
admin=["u30a77719df29231e45210d278bcfdb0b"]
...
wait = {
    ...
    'timeline':True,
    "Timeline":True,
    "comment":"لايك من طرف  Eric : https://line.me/ti/p/nt7sTXPHew",    
    "commentOn":True,
    "commentBlack":{},
...
#                            Name = summon(op.param2)
	    ...
	    if mid in op.param3:	        
		    ...
		    G = vipro.getGroup(op.param1)
                    if len(G.members) <= wait["memberscancel"]:
                        ...
                        vipro.acceptGroupInvitation(op.param1)
			vipro.sendText(op.param1,"هاااااااااااااااااااااااااااي")
	    ...
	    if mid in op.param3:
		    ...
		    G = vipro.getGroup(op.param1)
                    if len(G.members) <= wait["Members"]:
                        ...
                        vipro.acceptGroupInvitation(op.param1)
			vipro.sendText(op.param1,"هااااااااااااااااااااااااااااي")
	    else:
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     contact = vipro.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["Dont Tag!! ",cName + " Ngapain Ngetag?",cName + " fuck fuck","tagOff", cName + " wtf","bitchs " + cName, "loooool " + cName, "Kamu Siapa " + cName + "?", "Ada Perlu Apa " + cName + "?","Woii " + cName + " Jangan Ngetag, Riibut!"]
                     ret_ = random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention2"] == True:          
                    contact = vipro.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Sekali lagi nge tag gw Cium Meslah Ni Hhhh!","Hadir Bos Masih Otw Sabar Yach","Assalamualaikum " + cName + " Hadir Bos Masih Mandi Ganteng KkKk!"]
                    ret_ = random.choice(balas)
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                    mentionees = mention['MENTIONEES']
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention3"] == True:          
                    contact = vipro.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Assalamualikum " + cName + ", Kenapa Sayang Tag-Tag Kangen Yach!"]
                    balas1 = "😘😘😘. . ."
                    ret_ = random.choice(balas)
                    image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
                    name = re.findall(r'@(\w+)', msg.text)
        ...
        if op.type == 25:
          if wait["Bot"] == True:    
			...
			vipro.sendText(msg.to,"Admin Detected~")
                ...
                vipro.sendMessage(msg)
		vipro.sendText(msg.to,"ooooooowwwwiiiiii (^_^)")
		...
		vipro.sendText(msg.to,"Itu Yang Buat Grup Ini")
            ...
            if msg.contentType == 13:
                if wait['invite'] == True:
                     ...
                     for s in groups.members:
                         if _name in s.displayName:
                             vipro.sendText(msg.to, _name + " ok")
                         else:
            ...
            elif msg.text in ["Key creator","help creator","eric1"]:
                vipro.sendText(msg.to,creatorMessage)

            elif msg.text in ["Key group","help group","eric2"]:
                vipro.sendText(msg.to,groupMessage)

            elif msg.text in ["Key","help","eric"]:
                vipro.sendText(msg.to,helpMessage)

            elif msg.text in ["Key self","help self","eric4"]:
                vipro.sendText(msg.to,selfMessage)

            elif msg.text in ["Key bot","help bot","eric3"]:
                vipro.sendText(msg.to,botMessage)

            elif msg.text in ["Key set","help set","eric5"]:
                vipro.sendText(msg.to,setMessage)

            elif msg.text in ["Key media","help media","eric6"]:
                vipro.sendText(msg.to,mediaMessage)
                
            elif msg.text in ["Key admin","help admin","eric0"]:
		...
		gid = vipro.getGroupIdsJoined()
            ...
            elif msg.text in ["cancelall","ericfuck"]:
                if msg.toType == 2:
            ...
            elif msg.text in ["eric j on","Autojoin on"]:
		if msg.from_ in admin:
                    wait["AutoJoin"] = True
                    wait["AutoJoinCancel"] = False
                    vipro.sendText(msg.to,"Auto Join on")
		else:
		...
		if msg.from_ in admin:
                    ...
                    vipro.sendText(msg.to,"Auto Respon1 on")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")
		...
		if msg.from_ in admin:
                    ...
                    vipro.sendText(msg.to,"Auto Respon1 off")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")	
		...
		if msg.from_ in admin:
                    ...
                    wait["detectMention3"] = False
                    wait["kickMention"] = False
                    vipro.sendText(msg.to,"Auto Respon2 on")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")
            elif msg.text in ["Respon2 off"]:
		if msg.from_ in admin:
                    wait["detectMention2"] = False
                    vipro.sendText(msg.to,"Auto Respon2  Off")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")	
		...
		if msg.from_ in admin:
                    ...
                    wait["AutoKick"] = True
                    wait["Qr"] = True
                    vipro.sendText(msg.to,"All Protecton on")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")
		...
		if wait["Simi"] == True: md+="╠➩✔️ Simisimi : On\n"
            ...
            elif msg.text.lower() in ["eric","eric00","nono","haha"]:
                msg.contentType = 7
		...
		if msg.from_ in admin:	 	        
		    ...
		    vipro.sendText(msg.to,"Success Add all")
                ...
                wait["likeOn"] = True
                vipro.sendText(msg.to,"لايك يبوووي")                
		...
		vipro.sendText(msg.to,"Success recover")
                ...
                midd = msg.text.replace("Kick: ","براا")
		if midd not in admin:
		    ...
		    vipro.sendText(msg.to,"Admin Detected")
                ...
                midd = msg.text.replace("Invite: ","دعوة")
                vipro.findAndAddContactsByMid(midd)
                vipro.inviteIntoGroup(msg.to,[midd])

            elif "Invite eric" in msg.text:
                midd = "u186fe987ceb445ff5f001990fd583ff2"
                vipro.inviteIntoGroup(msg.to,[midd])
		...
		if msg.from_ in admin:
		    ...
		    if msg.from_ in Creator:
                     if msg.toType == 2:
                        ...
                        gs = vipro.getGroup(msg.to)
                        vipro.sendText(msg.to,"براااااااا")
                        targets = []
                        for g in gs.members:
